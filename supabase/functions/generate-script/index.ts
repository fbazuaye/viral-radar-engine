    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional YouTube scriptwriter. Given a video topic, create a complete video script outline with a powerful hook and 4-6 timed sections. The hook should be attention-grabbing and create curiosity. Each section should have a title with timestamp range and detailed content notes for what to cover.`
          },
        ...
        tools: [{
          ...
          function: {
            name: "create_script",
            description: "Return a video script outline",
            parameters: {
              type: "object",
              properties: {
                hook: { type: "string" },
